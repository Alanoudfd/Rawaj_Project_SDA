...
def _username_for(db: Session, restaurant: Restaurant) -> str:
    ...
    taken = db.scalar(select(UserAccount.id).where(UserAccount.username == base))
    return f"{base}-{restaurant.id}" if taken else base
...
def provision_access(restaurant_id: int, *, session_factory=None) -> dict | None:
    ...
    Safe to call again for the same restaurant: the username and password come out the same. (Changing
    ACCOUNT_SECRET reissues every password; the account is updated to match the newly derived one.)
    """
    ...
    with (session_factory or SessionLocal)() as db:
        ...
        db.commit()
        return {"username": account.username, "password": password, "login_url": login_url()}
