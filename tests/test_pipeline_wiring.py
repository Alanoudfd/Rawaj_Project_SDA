...
class ClientEmailSignInTests(Db):
    ...
    def test_email_contains_username_password_and_link(self):
        ...
        self.assertEqual(body.count("Congratulations"), 0)  # the model's text announces it; the footer does not repeat it
    ...
    def test_rebuilding_the_email_gives_the_same_credentials_and_they_sign_in(self):
        ...
        with self.sessions() as db:
            ...
            account = accounts.authenticate(db, "3brews.sa", accounts.client_password(1))
            self.assertEqual(account.restaurant_id, 1)

    def test_no_access_provider_leaves_the_email_as_before(self):
        instance = object.__new__(OutreachFollowUpWorkflow)
        instance.access_provider = None
        body = instance._append_trusted_dashboard_link(body="x", action=ActionType.NOTIFY_STRATEGY_READY, state=self.state())
...
class AccountAndAuthTests(Db):
    ...
    def test_login_route_for_owners_wrong_passwords_and_lockout(self):
        ...
        self.assertEqual(ok.json(), {"role": "owner", "username": "3brews.sa", "restaurant_id": 1})
        wrong = client.post("/api/auth/login", json={"username": "3brews.sa", "password": "nope"})
        ...
        for _ in range(accounts.MAX_FAILED_ATTEMPTS):
            client.post("/api/auth/login", json={"username": "3brews.sa", "password": "nope"})
        locked = client.post("/api/auth/login", json={"username": "3brews.sa", "password": access["password"]})
    ...
    def __init__(self):
        ...
        def resume(**kwargs):
            ...
            if kwargs["decision"] == "APPROVED":
                return SimpleNamespace(errors=[], execution=SimpleNamespace(status="SENT"), email_draft=SimpleNamespace(status=SimpleNamespace(value="SENT")))
...
class AutoApprovalTests(unittest.TestCase):
    """The restaurant is the only human in the loop: emails that passed the AI review go out on their own."""

    def sweep(self, *items, env=None, **settings):
        fake = FakeOutreach()
        fake.paused = {f"thread-{i}": item for i, item in enumerate(items)}
        for key, value in settings.items():
            setattr(fake.settings, key, value)
        with patch.dict(os.environ, env or {}):
            os.environ.pop("AUTO_APPROVE_EMAILS", None) if not env else None
            return fake, workflow.send_reviewed_emails(lambda: fake)

    def test_a_reviewed_email_is_approved_on_the_agents_behalf(self):
        fake, outcome = self.sweep(paused("m1"), paused("m2", "STRATEGY_READY_NOTIFICATION"))
        self.assertEqual([a["message_id"] for a in outcome["approved"]], ["m1", "m2"])
        first = fake.resumed[0]
        self.assertEqual((first["decision"], first["reviewer_id"], first["message_id"]), ("APPROVED", "auto:ai-review", "m1"))
        self.assertEqual((first["message_revision"], first["content_sha256"]), (1, "h" * 64))

    def test_nothing_is_approved_unless_the_review_passed_and_the_type_is_routine(self):
        fake, outcome = self.sweep(
            paused("bad-review", passed=False), paused("not-private", privacy_safe=False),
            paused("issue", "CLIENT_ISSUE_RESPONSE"), paused("ok"),
        )
        self.assertEqual([a["message_id"] for a in outcome["approved"]], ["ok"])
        self.assertEqual({s["message_id"] for s in outcome["skipped"]}, {"bad-review", "not-private", "issue"})
        self.assertEqual([r["message_id"] for r in fake.resumed], ["ok"])

    def test_it_can_be_switched_off_and_needs_an_email_provider(self):
        fake, outcome = self.sweep(paused("m1"), env={"AUTO_APPROVE_EMAILS": "false"})
        self.assertFalse(outcome["enabled"])
        self.assertEqual(fake.resumed, [])
        fake, outcome = self.sweep(paused("m1"), email_provider=None)
        self.assertEqual((outcome["approved"], fake.resumed), ([], []))
        self.assertIn("not configured", outcome["reason"])

    def test_one_failure_does_not_stop_the_others(self):
        fake = FakeOutreach()
        fake.paused = {"t1": paused("boom"), "t2": paused("fine")}
        original = fake.workflow.resume_human_approval
        ...
        fake.workflow.resume_human_approval = flaky
        outcome = workflow.send_reviewed_emails(lambda: fake)
        self.assertEqual([a["message_id"] for a in outcome["approved"]], ["fine"])
        self.assertEqual(outcome["skipped"][0]["message_id"], "boom")

    def test_an_approved_email_is_not_approved_twice(self):
        fake, first = self.sweep(paused("m1"))
        fake.paused = {}  # the graph moved on, so it is no longer waiting
        again = workflow.send_reviewed_emails(lambda: fake)
        self.assertEqual((len(first["approved"]), again["approved"]), (1, []))
